...
private:
    void checkLine() {
        ...
        if (index_ >= expected_.size()) {
            ...
            if (++mismatch_count_ >= mismatch_limit_) exit(0);
            line_.clear();
            ++index_;
            return;
        }
    ...
    const size_t mismatch_limit_ = 1;
};*/
/*
...
public:
    ...
    virtual string get_morale(){return "";}
    virtual ostream& get_weapon_infor(ostream& oerr){
        vector<Weapon> bagg = this->pass_weapon();
        vector<pair<string,int>> outt_inf;
        for (size_t num=0;num<bagg.size();num++) {
            if (bagg[num].id == ARROW) {
                if (bagg[num].arrow_cnt < 3)
                outt_inf.push_back(make_pair("arrow",3 - bagg[num].arrow_cnt));
            }
        }
        for (size_t num=0;num<bagg.size();num++) {if (bagg[num].id == BOMB) outt_inf.push_back(make_pair("bomb",-1));}
        for (size_t num=0;num<bagg.size();num++) {if (bagg[num].id == SWORD) outt_inf.push_back(make_pair("sword",bagg[num].sword_atk));}
        if (outt_inf.empty()) {
            oerr<<"no weapon"<<endl;
            return oerr;
        }
        for (size_t len=0;len<outt_inf.size();len++) {
            oerr<<outt_inf[len].first;
            if (outt_inf[len].second > 0) oerr<<"("<<outt_inf[len].second<<")";
            if (len + 1 < outt_inf.size()) oerr<<",";
        }
        oerr << endl;
        return oerr;
    }
...
public:
    ...
    vector<Weapon> pass_weapon() {return {wp};}
    void reset_arrow() {
        if (wp.id == ARROW && wp.arrow_cnt == 3) wp = Weapon();
    }
    ...
    string get_morale(){return "morale "+to_string(this->morale);}
};
...
public:
    ...
    Weapon wp1,wp2;
    void reset_arrow() {
        if (wp1.id == ARROW && wp1.arrow_cnt == 3) wp1 = Weapon();
        if (wp2.id == ARROW && wp2.arrow_cnt == 3) wp2 = Weapon();
    }
...
public:
    ...
    void reset_arrow() {
        if (wp1.id == ARROW && wp1.arrow_cnt == 3) wp1 = Weapon();
        if (wp2.id == ARROW && wp2.arrow_cnt == 3) wp2 = Weapon();
        if (wp3.id == ARROW && wp3.arrow_cnt == 3) wp3 = Weapon();
    }
    ...
    if (hq.life >= cost[order[hq.nextIndex]]) {
        ...
        switch (type) {
            case DRAGON : {
                ...
                warrior = std::move(w);
                if (hq.side == "red") {
                    war_map[0].red_warrior = std::move(warrior);
                } else {
                    war_map[N + 1].blue_warrior = std::move(warrior);
                }
                break;
            }
            case NINJA : {
    ...
    if (lion->loyality <= 0) {
        if (hq.side == "red") {
            if (situation != N+1) {
                ...
                lion->escape = true;
                (*warrior).reset();
                cout << timePrefix(time_index) << hq.side << ' ' << "lion " << idx << " ran away\n";
            }
        } 
        else {
            if (situation != 0) {
...
void Warrior_March(int time_index) {
    ...
    Snapshot red_reached_blue{};
    Snapshot blue_reached_red{};
    bool has_red_reached_blue = false;
    bool has_blue_reached_red = false;
    // 红方武士前进
    for (int pos=N+1; pos>0; pos--) {
        ...
        if (cell.red_warrior) {
            ...
            war_map[pos].red_warrior = std::move(war_map[pos-1].red_warrior);
            if (pos >= 1 && pos <= N) {
    ...
    for (int pos=0; pos<=N; pos++) {
        ...
        if (cell.blue_warrior) {
            ...
            war_map[pos].blue_warrior = std::move(war_map[pos+1].blue_warrior);
            if (pos >= 1 && pos <= N) {
...
*/
    if (has_blue_reached_red) {
        int life = blue_reached_red.life_value;
        int force = blue_reached_red.attack_value;
        if (war_map[0].blue_warrior) {
            life = war_map[0].blue_warrior->life_value;
            force = war_map[0].blue_warrior->attack_value;
        }
        cout << timePrefix(time_index) << "blue " << warriorName[blue_reached_red.type] << ' ' << blue_reached_red.index
             << " reached red headquarter with " << life << " elements and force " << force << '\n';
    ...
    if (has_red_reached_blue) {
        int life = red_reached_blue.life_value;
        int force = red_reached_blue.attack_value;
        if (war_map[N+1].red_warrior) {
            life = war_map[N+1].red_warrior->life_value;
            force = war_map[N+1].red_warrior->attack_value;
        }
        cout << timePrefix(time_index) << "red " << warriorName[red_reached_blue.type] << ' ' << red_reached_blue.index
             << " reached blue headquarter with " << life << " elements and force " << force << '\n';
...
/*
    for (int i=1;i<=N;i++) {
        if (war_map[i].red_warrior) cerr<<"red "<<Map[war_map[i].red_warrior->type]<<" "<<war_map[i].red_warrior->index<<" in city "+to_string(i)<<endl;
        if (war_map[i].blue_warrior) cerr<<"blue "<<Map[war_map[i].blue_warrior->type]<<" "<<war_map[i].blue_warrior->index<<" in city "+to_string(i)<<endl;
    }
*/
}
...
void throw_arrow(int time_index){
    ...
    for (int i=1;i<=N;i++) {
        if (war_map[i].red_warrior) {
            //cerr << war_map[i].red_warrior->side << " " << Map[war_map[i].red_warrior->type] << " " << war_map[i].red_warrior->index << " in city " << i << ' ';
            //war_map[i].red_warrior->get_weapon_infor(cerr);
            if (i < N) {
                    ...
                    switch (t) {
                        case DRAGON : {
                            ...
                            if (dragon->wp.id == ARROW) {
                                //cerr << "DEBUG_ARROW: red dragon "<<dragon->index<<" arrow_cnt="<<dragon->wp.arrow_cnt<<" target_pos="<<i+1<<" target_life="<<war_map[i+1].blue_warrior->life_value<<"\n";
                                if (dragon->wp.arrow_cnt == 3) dragon->wp = Weapon();
                                    ...
                                    if (war_map[i+1].blue_warrior->life_value <= 0) {
                                        ...
                                        war_map[i+1].blue_warrior->dead_because_of_arrow = true;
                                    }
        ...
        // ! 注意在40分钟的时候统一将beaten_by_arrow属性更新为false !!!
                        ...
                        case ICEMAN : {
                            //cerr << "[blue dragon debug] " << "blue " << Map[war_map[i+1].blue_warrior->type] << " " <<war_map[i+1].blue_warrior->index << " was here in city " + to_string(i+1) << endl;
        ...
        if (war_map[i].blue_warrior) {
            //cerr << war_map[i].blue_warrior->side << " " << Map[war_map[i].blue_warrior->type] << " " << war_map[i].blue_warrior->index << " in city " << i << ' ';
            //war_map[i].blue_warrior->get_weapon_infor(cerr);
            if (i > 1) {
                ...
                if (war_map[i-1].red_warrior) {
                    ...
                    switch (t) {
                        case DRAGON : {
                            auto *dragon = dynamic_cast<Dragon*>(w.get());
                            if (dragon->wp.id == ARROW) {
                                //cerr << "DEBUG_ARROW: blue ninja "<<dragon->index<<" arrow_cnt="<<dragon->wp.arrow_cnt<<" target_pos="<<i-1<<" target_life="<<war_map[i-1].red_warrior->life_value<<"\n";
                                if (dragon->wp.arrow_cnt == 3) dragon->wp = Weapon();
                                else {
                                    dragon->wp.arrow_cnt ++;
                                    ...
                                    if (war_map[i-1].red_warrior->life_value <= 0) {
                                        ...
                                        war_map[i-1].red_warrior->dead_because_of_arrow = true;
                                    }
                                    action_lst.push_back(temp);
                                    temp.clear();
                                }
        ...
        // ! 注意在40分钟的时候统一将beaten_by_arrow属性更新为false !!!
                        ...
                        case NINJA : {
                            auto *ninja = dynamic_cast<Ninja*>(w.get());
                            if (ninja->wp1.id == ARROW) {
                                int count = ninja->wp1.arrow_cnt;
                                //cerr << "DEBUG_ARROW: blue dragon "<<ninja->index<<" arrow_cnt1="<<ninja->wp1.arrow_cnt<<" arrow_cnt2="<<ninja->wp2.arrow_cnt<<" target_pos="<<i-1<<" target_life="<<war_map[i-1].red_warrior->life_value<<"\n";
                                if (ninja->wp1.arrow_cnt == 3) ninja->wp1 = Weapon();
                                else {
                                    ninja->wp1.arrow_cnt ++;
...
*/
    ...
    for (int i=1;i<=N;i++) {
        auto &rptr = war_map[i].red_warrior, &bptr = war_map[i].blue_warrior;
        if ((rptr && bptr) && (!rptr->dead_because_of_arrow && !bptr->dead_because_of_arrow)) {
            if (rptr->life_value <= 0 || bptr->life_value <= 0) continue;
            bool bombed = false;
            ...
            if ((war_map[i].flag == 1) || (war_map[i].flag == 0 && i % 2 == 1)) {
                ...
                int blue_ctk = 0;
                // 如果对方是 ninja，则不会反击
                if (bptr->type != NINJA) blue_ctk = bptr->attack_value / 2 + bptr->get_sword_atk();
            ...
            if (bombed) continue;
            if ((war_map[i].flag == -1) || (war_map[i].flag == 0 && i % 2 == 0)) { // ninja挨打不还手
                // 蓝武士主动攻击
                int blue_atk = bptr->attack_value + bptr->get_sword_atk();
            ...
            if ((war_map[i].flag == -1) || (war_map[i].flag == 0 && i % 2 == 0)) {
                ...
                int red_ctk = 0;
                // 如果对方是 ninja，则不会反击
                if (rptr->type != NINJA) red_ctk = rptr->attack_value / 2 + rptr->get_sword_atk();
...
void battle(int time_index) {
    ...
    for (int pos = 1; pos <= N ; pos ++) {
        size_t event_start = action_lst.size();
        auto &rptr = war_map[pos].red_warrior;
        auto &bptr = war_map[pos].blue_warrior;
        bool element_added = false;
        element_aquire element_tmp("","",0);
        if (rptr && bptr) { // 有双方武士，但生死不明
            
            // lion 若是战死，则其战斗前的生命值就会转移到对手身上。所谓“战斗前”，就是每个小时的40分前的一瞬间。
            int red_before_element = (rptr->type == LION) ? max(0, rptr->life_value) : 0;
            int blue_before_element = (bptr->type == LION) ? max(0, bptr->life_value) : 0;
            ...
            */
            ...
            if (rptr->life_value <= 0 || bptr->life_value <= 0) /*武士战死*/ {
                if (rptr->life_value <= 0) { // 蓝方胜利
                    ...
                    if (bptr->type == DRAGON) {
                        bptr->morale += 0.2;
                        if ((war_map[pos].flag == -1) || (war_map[pos].flag == 0 && pos % 2 == 0)) bptr->dragon_attack_first = true;
                    }
                ...
                else if (bptr->life_value <= 0) { // 红方胜利
                    rptr->life_value += blue_before_element;
                    war_map[pos].second_battle_result = 1;
                    if (rptr->type == DRAGON) {
                        rptr->morale += 0.2;
                        if ((war_map[pos].flag == 1) || (war_map[pos].flag == 0 && pos % 2 == 1)) rptr->dragon_attack_first = true;
                    }
                    if (rptr->type == WOLF) {
...
// dragon 在一次在它主动进攻的战斗结束后，如果还没有战死，而且士气值大于0.8，就会欢呼
            if (rptr) {
                //cerr << "red " + Map[rptr->type] + " " +to_string(rptr->index) << rptr->get_morale() + " " << rptr->dragon_attack_first<<endl;
            ...
            if (bptr) {
                //cerr << "blue " + Map[bptr->type] + " " + to_string(bptr->index) << bptr->get_morale() + " " << bptr->dragon_attack_first<<endl;
...
// 如果某武士在某城市的战斗中杀死了敌人，则该武士的司令部立即取得该城市中所有的生命元。注意，司令部总是先完成全部奖励工作，然后才开始从各个打了胜仗的城市回收生命元
            if (war_map[pos].second_battle_result != 0 && element_added) {
                event e(3,element_tmp.war,element_tmp.num,pos);
                action_lst.push_back(e);
            }
// 旗帜升起
            int judge = war_map[pos].flag;
            if (war_map[pos].first_battle_result == war_map[pos].second_battle_result) {
                if (war_map[pos].first_battle_result == 1 && judge != 1) {
                    war_map[pos].flag = 1;
                    event e(4,"red",pos);
                    action_lst.push_back(e);
                }
                if (war_map[pos].second_battle_result == -1 && judge != -1) {
                    war_map[pos].flag = -1;
                    event e(4,"blue",pos);
                    action_lst.push_back(e);
                }
        ...
        // Reorder events produced for this city so that attack/counterattack events
        // remain first (in original order) and kill/yell/earned/flag events follow.
        size_t event_end = action_lst.size();
        if (event_end > event_start) {
            vector<event> city_events(action_lst.begin() + event_start, action_lst.begin() + event_end);
            // erase the range
            action_lst.erase(action_lst.begin() + event_start, action_lst.begin() + event_end);
            // stable partition: keep who_first in {1,0,-1} first, others after
            vector<event> primary, secondary;
            for (const auto &ev : city_events) {
                if (ev.who_first == 1 || ev.who_first == 0 || ev.who_first == -1) primary.push_back(ev);
                else secondary.push_back(ev);
            }
            // append back preserving relative order
            action_lst.insert(action_lst.end(), primary.begin(), primary.end());
            action_lst.insert(action_lst.end(), secondary.begin(), secondary.end());
        }
...
void warrior_report(int time_index) {
    for (int pos=0;pos<=N+1;pos++) {
        if (war_map[pos].red_warrior) {
            ...
            for (size_t num=0;num<bag.size();num++) {
                if (bag[num].id == ARROW) {
                    if (bag[num].arrow_cnt < 3)
                    out_inf.push_back(make_pair("arrow",3 - bag[num].arrow_cnt));
                }
                else {
                    w->reset_arrow();
                }
