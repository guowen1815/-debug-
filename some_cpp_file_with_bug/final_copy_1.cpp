...
struct event {
    ...
    event(int a1,const unique_ptr<Warrior>&rptr,const unique_ptr<Warrior>&bptr,int a2):who_first(a1),locate(a2){
        redw = rptr->side + " " + Map[rptr->type];red_idx = rptr->index;bluew = rptr->side + " " + Map[bptr->type];blue_idx = bptr->index;
    ...
    for (int pos = 1; pos <= N ; pos ++) {
        ...
        if (rptr && bptr) { // 有双方武士，但生死不明
            ...
            */
            ...
            if (rptr->life_value > 0 && bptr->life_value > 0) /*幸存*/ { //平局
